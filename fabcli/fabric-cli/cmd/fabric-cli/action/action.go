...
import (
	...
	cliconfig "github.com/blockchain/fabric-sdk-go/fabcli/fabric-cli/cmd/fabric-cli/config"
	"github.com/blockchain/fabric-sdk-go/fabcli/fabric-cli/cmd/fabric-cli/printer"
...
func (action *Action) Initialize(flags *pflag.FlagSet) error {
	...
	action.flags = flags
	fmt.Println("Action ConfigFileFlag",cliconfig.ConfigFileFlag)
	fmt.Println("Action InitConfigFile",cliconfig.InitConfigFile)
	...
	//fmt.Println("Action init selectprovider",cliconfig.Config().SelectionProvider())
	...
	ctx, err := sdk.Context()()
	//fmt.Println("Action init ctx",ctx)
	...
	//fmt.Println("Action init endpoint",action.endpointConfig)


	networkConfig := action.endpointConfig.NetworkConfig()
    //fmt.Println("Action init network",networkConfig)

	level := levelFromName(cliconfig.Config().LoggingLevel())
    //fmt.Println("Action init level",level)
    ...
    //fmt.Println("Action init allpeers",allPeers)
	...
	fmt.Println("Action init peers",peers)
	...
	//fmt.Println("Action init userstore")
...
func (action *Action) OrgAdminChannelClient(orgID string) (*channel.Client, error) {
    ...
    fmt.Println("OrgAdminchannel",orgID)
...
func (action *Action) User() (mspapi.SigningIdentity, error) {
    ...
    fmt.Println("userName:",userName,"org",action.OrgID())
	if userName == "" {
...
func (action *Action) OrgUser(orgID, username string) (mspapi.SigningIdentity, error) {
	...
	user, err := mspClient.GetSigningIdentity(username)
	fmt.Println("user id and name",orgID,username,user)
	if err != nil {
		return nil, errors.Errorf("GetSigningIdentity returned error: %v", err)
	}
...
func (action *Action) OrgAdminUser(orgID string) (mspapi.SigningIdentity, error) {
	userName := cliconfig.Config().UserName()
	fmt.Println("OrgAdminUser:",userName,"orgID",orgID)
	if userName == "" {
		userName = adminUser
	}
